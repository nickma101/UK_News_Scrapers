...
def get_articles():
    ...
    for d in data:
        if d.get('primaryCategory') not in topics:
            data.remove(d)
    ...
    for d in data:
        if datetime.strptime(d.get('datePublished'), '%Y-%m-%d %H:%M:%S') < yesterday:
            data.remove(d)
    for d in data:
        articles.append(d)
    # some code that picks two dissimlar articles per topic
    return articles
